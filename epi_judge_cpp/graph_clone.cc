...
GraphVertex* CloneGraph(GraphVertex* graph) {
	unordered_map<GraphVertex*, GraphVertex*> vertices;
	queue<GraphVertex*> Q;
	Q.emplace(graph);
	vertices.emplace(graph, new GraphVertex({ graph->label }));
	while (!Q.empty()) {
		auto cur = Q.front();
		auto newCur = vertices[cur];
		Q.pop();
		for (auto e : cur->edges) {
			if (!vertices.count(e)) {
				vertices.emplace(e, new GraphVertex({ e->label }));
				Q.emplace(e);
			}
			newCur->edges.emplace_back(vertices[e]);
		}
	}
