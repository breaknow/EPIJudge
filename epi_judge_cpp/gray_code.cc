...
bool dfs(int pos, int n, vector<int> &ans, vector<bool> &chk) {
	if (pos == 0) {
		chk[0] = true;
		ans.emplace_back(0);
		return dfs(pos + 1, n, ans, chk);
	}
	if (pos == n) {
		if (Similar(ans[0], ans[n - 1]))
			return true;
		return false;
	}
	for (int i = 1; i < n; i *= 2) {
...
vector<int> GrayCode(int num_bits) {
	vector<int> ans;
	vector<bool> chk(pow(2, num_bits), false);
	dfs(0, pow(2, num_bits), ans, chk);
	return ans;
}
