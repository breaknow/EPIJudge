...
Rect IntersectRectangle(const Rect& R1, const Rect& R2) {
	...
	if (!((R1.x > R2.x + R2.width) || (R1.x + R1.width < R2.x))) {
		ans.x = (R1.x > R2.x) ? R1.x : R2.x;
		ans.width = ((R1.x + R1.width < R2.x + R2.width) ? R1.x + R1.width : R2.x + R2.width) - ans.x;
	}
	if (!((R1.y > R2.y + R2.height) || (R1.y + R1.height < R2.y))) {
		ans.y = (R1.y > R2.y) ? R1.y : R2.y;
		ans.height = ((R1.y + R1.height < R2.y + R2.height) ? R1.y + R1.height : R2.y + R2.height) - ans.y;
	}
	if(ans.width == -1 || ans.height == -1)
		return { 0, 0, -1, -1 };
	return ans;
}
